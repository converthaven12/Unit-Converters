...
const temperatureIntervalUnits: Record<string, { toK: (v: number) => number; fromK: (v: number) => number; }> = {
  "Celsius (°C)": {
    toK: (v) => v + 273.15,
    fromK: (v) => v - 273.15,
  },
...
const TemperatureIntervalConverterPage: React.FC = () => {
  const convert = (value: number, from: string, to: string): number => {
    if (!(from in temperatureIntervalUnits) || !(to in temperatureIntervalUnits)) return NaN;
  ...
  return (
    <ReusableConverter
      heading="Temperature Interval Converter"
