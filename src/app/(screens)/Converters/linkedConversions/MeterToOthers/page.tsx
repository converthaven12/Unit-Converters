...
const convert = (value: number, from: string, to: string): number => {
  const fromInMeters = value; // already in meters
  const toFactor = lengthToMeter[to];
  if (!toFactor) return 0;
  return fromInMeters / toFactor;
};

// Filter out the source unit 'Meter'
const toUnits = Object.keys(lengthToMeter).filter(unit => unit !== "Meter").sort();

const MeterToAllPage = () => {
  return (
    <LockedUnitConverter
      heading="Meter to All Units Converter"
      lockedFromUnit="Meter"
      units={toUnits}
      convert={convert}
    />
  );
