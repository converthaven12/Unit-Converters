...
const volumeToMilliliter: Record<string, number> = {
  'Cubic Meter': 1e6,
...
const convert = (value: number, from: string, to: string): number => {
  const fromInMilliliters = value;
  const toFactor = volumeToMilliliter[to];
  if (!toFactor) return 0;
  return fromInMilliliters / toFactor;
};

const toUnits = Object.keys(volumeToMilliliter).sort();

const MilliliterToAllPage = () => {
  return (
    <LockedUnitConverter
      heading="Milliliter to All Units Converter"
      lockedFromUnit="Milliliter"
      units={toUnits}
      convert={convert}
    />
  );
...
export default MilliliterToAllPage;