...
const convert = (value: number, from: string, to: string): number => {
  const fromInMeters = value * lengthToMeter[from]; // Kilometer to meter
  const toFactor = lengthToMeter[to];
  if (!toFactor) return 0;
  return fromInMeters / toFactor;
};

// Prepare all unit keys (excluding 'Kilometer')
const toUnits = Object.keys(lengthToMeter).filter(unit => unit !== 'Kilometer').sort();

const KmToAllPage = () => {
  return (
    <LockedUnitConverter
      heading="Kilometer to All Units Converter"
      lockedFromUnit="Kilometer"
      units={toUnits}
      convert={convert}
    />
  );
